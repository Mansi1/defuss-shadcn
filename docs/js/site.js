...
(function () {
    ...
    function initVirtualListDemos() {
        ...
        if (!ns || !ns.virtualList)
            return;
        if (!document.getElementById('demo-virtual-list'))
        ...
        [['demo-virtual-list', 10000000], ['demo-vl-sm', 1000], ['demo-vl-lg', 1000], ['demo-vl-jump', 10000000]]
            ...
            if (el)
                ns.virtualList.setData(el, pair[1], renderRow);
        });
        ...
        document.querySelectorAll('[data-vl-jump]').forEach(function (btn) {
            if (btn.dataset.vlInit)
                return;
            btn.dataset.vlInit = '';
            btn.addEventListener('click', function () {
                var list = document.getElementById('demo-vl-jump');
                if (list && list.api)
                    list.api.setState('default', { index: Number(btn.dataset.vlJump) });
            });
        });
    }
